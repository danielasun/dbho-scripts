...
Human_x =
{
	...
	Properties =
	{
	    ...
	    Behavior =
	    {
	      ...
	      fHungerFull = 100,
	      fHungerThreshold = 30,
	      fHungerRate = 5,  -- deplete per second
	    },
...
function Human_x:OnResetCustom()
  ...
  -- Reset hunger
  if (self.Properties.Behavior.bEnableHunger) then
    self.gameParams.hunger = self.Properties.Behavior.fHungerFull;
    self.gameParams.bHungry = false;
  end
end

function Human_x.Server:OnUpdate(dt)
  if (System.IsEditing() or self:IsDead()) then
    return 
  end

  -- Hunger bar
  if (self.Properties.Behavior.bEnableHunger) then
    if (self.gameParams.hunger > 0) then
      self.gameParams.hunger = math.max(0, self.gameParams.hunger - self.Properties.Behavior.fHungerRate * dt);
      --System.Log(tostring(self.gameParams.hunger));
    end

    if (self.gameParams.hunger < self.Properties.Behavior.fHungerThreshold) then
      if (not self.gameParams.bHungry) then
        self.gameParams.bHungry = true
        AI.ModifySmartObjectStates(self.id, "Hungry");
      end
    end
  end
end
...
-- FlowEvents
--------------------------------------------------
function Human_x:Event_SetTargetPos(sender, params)
  -- Movement can be handled in a flowgraph tied to the entity itself
  self:ActivateOutput("TargetPos", params)
end

function Human_x:Event_Eat(sender, params)
  if (self.Properties.Behavior.bEnableHunger) then
    self.gameParams.hunger = math.min(self.gameParams.hunger + params, self.Properties.Behavior.fHungerFull);
...
Human_x.FlowEvents =
{
  Inputs =
  {
    Eat = {Human_x.Event_Eat, "float" },
    SetTargetPos = { Human_x.Event_SetTargetPos, "Vec3" },
  },
  Outputs =
  {
    TargetPos = "Vec3",
